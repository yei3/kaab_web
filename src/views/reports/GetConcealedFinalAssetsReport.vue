...
<script>
  ...
  export default {
    ...
    props: {
      caption: {
        type: String,
        default: 'Reporte de activos conciliados'
      },
      hover: {
        type: Boolean,
        default: true
    ...
    async mounted() {
      ...
      //const assets = await reports.getConcealedFinalAssetsReport(this.$session.get('projectID'),null,this.deptTree);
    ...
    methods: {
      ...
      exportEx(){
        var expData = [];
        expData.push(["Reporte de Activos Conciliados","","","","","","","","",""]);
        ...
        XLSXjs.utils.book_append_sheet(wb, report, 'Conciliados')
        XLSXjs.writeFile(wb, 'conciliados.xlsx')
      },
      ...
      async search(){
        this.flag = false;
        const assets = await reports.getConcealedFinalAssetsReport(this.$session.get('projectID'),this.departmentID,this.deptTree);
        var localitems = [];
