...
<script>
  ...
  export default {
    ...
    props: {
      caption: {
        type: String,
        default: 'Reporte de activos con etiqueta sin datos'
      },
      hover: {
        type: Boolean,
        default: true
    ...
    async mounted() {
      ...
      //const assets = await reports.getNoDataLabelFinalAssetsReport(this.$session.get('projectID'),null,this.deptTree);
    ...
    methods: {
      ...
      exportEx(){
        var expData = [];
        expData.push(["Reporte de Activos con Etiqueta sin Datos","","","","","","","","",""]);
        ...
        XLSXjs.utils.book_append_sheet(wb, report, 'Sin etiqueta')
        XLSXjs.writeFile(wb, 'sin_etiqueta.xlsx')
      },
      ...
      async search(){
        this.flag = false;
        const assets = await reports.getNoDataLabelFinalAssetsReport(this.$session.get('projectID'),this.departmentID,this.deptTree);
        var localitems = [];
