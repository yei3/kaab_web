      ...
      <transition name="slide">
        <b-card>
          ...
          <b-table :busy="!flag"  :hover="hover" :striped="striped" :bordered="bordered" :small="small" :fixed="fixed" responsive="lg" :items="items" :fields="fields" :current-page="currentPage" :per-page="perPage" style="overflow-x: scroll">
            ...
            <template slot="initialLocation" slot-scope="data">
              {{data.item.initialLocation}}
            </template>
...
<script>
  ...
  export default {
    ...
    props: {
      caption: {
        type: String,
        default: 'Reporte de activos con cambio de ubicación'
      },
      hover: {
        type: Boolean,
        default: true
      ...
      return {
        ...
        fields: [
          ...
          {label: 'Nombre Unidad', key: 'location', sortable: true},
           {label: 'Ubicación inicial', key: 'initialLocation', sortable: true},
    ...
    async mounted() {
      ...
      //const assets = await reports.getLocationChangedFinalAssetsReport(this.$session.get('projectID'),null,this.deptTree);
    ...
    methods: {
      ...
      exportEx(){
        var expData = [];
        expData.push(["Reporte de Activos con Cambio de Ubicación","","","","","","","","",""]);
        ...
        XLSXjs.utils.book_append_sheet(wb, report, 'Cambio de Ubicación')
        XLSXjs.writeFile(wb, 'cambio_ubicacion.xlsx')
      },
      ...
      async search(){
        this.flag = false;
        const assets = await reports.getLocationChangedFinalAssetsReport(this.$session.get('projectID'),this.departmentID,this.deptTree);
        var localitems = [];
        ...
        assets.data.finalAssets.forEach(function(finalAsset){
          localitems.push({
            ...
            initialLocation: localLocations.find(dp => dp.id === finalAsset.initialLocation) === undefined ? "" : localLocations.find(dp => dp.id === finalAsset.initialLocation).name,
